{ clangStdenv, fetchFromGitHub, libpcap, libbpf, libbsd, openssl,
  dpdk, numactl, makeWrapper, lib, iproute }:

let
  repo = import ./repo.nix { inherit fetchFromGitHub; };
in clangStdenv.mkDerivation (repo // {
  pname = "freerouter-native";

  buildInputs = [ makeWrapper libpcap libbpf libbsd openssl dpdk numactl ];
